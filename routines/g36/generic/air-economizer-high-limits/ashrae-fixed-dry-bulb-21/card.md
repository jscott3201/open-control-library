# ASHRAE 90.1 fixed 21 °C dry-bulb economizer high limit

| Field | Value |
|---|---|
| Routine ID | `G36-GEN-AEHL__ashrae-fixed-dry-bulb-21` |
...
temperature cutoff for the ASHRAE 90.1 fixed dry-bulb 21 °C bucket. The source
groups climate zones 5A and 6A in this bucket. The fixture selects zone 5A as
its representative fixed configuration.
...
`Buildings.Controls.OBC.CDL.Reals.Sources.Constant` instance `con1` with
`k=294.15 K`. The routine has no boundary inputs, state, optional connectors,
...
object and expects `TCut = 294.15 K` with zero absolute tolerance. The graph and
...
- Canonical class: partial; 9 of 12 donor variants are present.
